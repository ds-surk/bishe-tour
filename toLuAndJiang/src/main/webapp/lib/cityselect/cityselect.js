        ...
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                if(typeof  that.callback =="function"){
                    that.callback.call(that.input,this.innerHTML);
                }else{
                	
            /*     	获得值*/
                that.input.value = this.innerHTML;
                var obj = this.innerHTML;  /*$("#travelCitySelect1").val()*/
                console.log("点击事件获取值的时候1:"+obj);
              /*  $.ajax({
        url:"getCountry",                 
        type:'post',请求方式 还可以是get type不可写成Type 不让会导致数据发送不过去,使用post无法接受 
         dataType:'travelmessage',　　　　返回格式 ,还可以是json
                ...
                $.ajax({
                	  ...
                	  /* contentType: 'application/json; charset=UTF-8',*/
                	   success:function(data){
           			    ...
           			    /*alert(box);*/
           			    box.innerHTML="";
           			    /*alert(document.getElementById("zhoubian1").innerHTML);*/
           			    
           			 var box1="";
           			    
           			 for(var i=0;i<data.length;i++){          				
           				box1+="<c:if test='${i%2==1}'>"+
								"<div class='list-body-column'>"+
							"</c:if>"+
							 "<c:if test='${i%2==0}'><div class='column-padding'></div></c:if>"+
							"<div class='column'>"+
								"<c:if test='${i%2==1}'>"+
								...
								"<div class='column-desc'>"+
									"<div class='cell-desc' title="+data[i].travelname+">"+
										data[i].travelname+"</div>"+
